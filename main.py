...
class SDGenerator(Star):
    ...
    async def show_conf(self, event: AstrMessageEvent):
        ...
        try:
            ...
            positive_prompt1 = self.config.get("prompt_group").get("positive_prompt_group").get("positive_prompt1", "无法读取")   # 获取正面提示词1
            negative_prompt1 = self.config.get("prompt_group").get("nagative_prompt_group").get("negative_prompt1", "无法读取")  # 获取负面提示词1
